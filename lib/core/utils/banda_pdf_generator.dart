...
class BandaPdfGenerator {
  ...
  static pw.Widget _buildTablaDesglose(BandaSection section) {
    final componentesConDatos = section.components
        .where(
          (c) =>
      ...
      children: [
        ...
        pw.Table(
          ...
          children: [
            ...
            pw.TableRow(
              children: [
                pw.Container(
                  ...
                  child: _renderComponentBlock(leftGroup),
                ),
                pw.Container(
                  height: 250,
                  padding: const pw.EdgeInsets.all(5),
                  child: _renderComponentBlock(rightGroup),
                ),
              ],
            ),
            pw.TableRow(
            ...
            pw.TableRow(
              children: [
                _buildComentariosPorSeccion(leftGroup),
                _buildComentariosPorSeccion(rightGroup),
              ],
            ),
            pw.TableRow(
              decoration: pw.BoxDecoration(color: _kGreyHeader),
            ...
            pw.TableRow(
              children: [
                pw.Container(
                  height: 80,
                  child: _buildAccionesPorSeccion(leftGroup),
                ),
                pw.Container(
                  height: 80,
                  child: _buildAccionesPorSeccion(rightGroup),
                ),
              ],
            ),
          ],
  ...
  // static pw.Widget _buildNestedTableColumn(
  //   List<BandaComponent> components, {
  ...
  static pw.Widget _buildAccionesPorSeccion(List<BandaComponent> components) {
    final conAcciones = components
        .where((c) => c.observation.trim().isNotEmpty)
        .toList();

    if (conAcciones.isEmpty) {
      return pw.Padding(
        padding: const pw.EdgeInsets.all(5),
    ...
    return pw.Padding(
      ...
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: conAcciones
            .map(
              (c) => pw.Padding(
                padding: const pw.EdgeInsets.only(bottom: 4),
                child: pw.Text(
                  "${c.name.toUpperCase()}: ${c.observation}",
  ...
  static pw.Widget _buildComentariosPorSeccion(
    List<BandaComponent> components,
  ) {
    final componentesConComentarios = components
    ...
    return pw.Wrap(
      ...
      children: [
        ...
        ...opcionesFijas.map((opt) {
          ...
          final isSelected =
              cleanSelected.contains(opt.id.trim().toLowerCase()) ||
  ...
  static pw.Widget _buildTechnicalFooter(
    Map<String, dynamic> data,
    List<Roller> rodillos,
  ) {
    return pw.Table(
  ...
  static pw.Widget _buildRodillosDanadosTable(List<Roller> datosRodillos) {
    ...
    for (var r in datosRodillos) {
      ...
      final int posicionesActivas =
          (r.isLeft ? 1 : 0) + (r.isCenter ? 1 : 0) + (r.isRight ? 1 : 0);
  ...
  static pw.Widget _buildRodilleriaFullPage(
    Map<String, dynamic> data,
    List<Roller> rodillos,
  ) {
        ...
        pw.Table(
          ...
          columnWidths: {
            ...
            ...List.generate(
              7,
              (i) => i + 2,
            ).asMap().map((k, v) => MapEntry(v, const pw.FlexColumnWidth(1))),
          },
          children: [
            pw.TableRow(
              decoration: pw.BoxDecoration(color: _kGreyHeader),
              children: [
                _cell("No. MESA", bold: true),
                _cell("No. BASE", bold: true),
                _cell("IZQ", bold: true),
                _cell("CEN", bold: true),
                _cell("DER", bold: true),
                _cell("IMPACTO", bold: true),
                _cell("RETORNO", bold: true),
                _cell("SOPORTE", bold: true),
              ],
            ),
  ...
  static pw.TableRow _resumenRow(String desc, String total) => pw.TableRow(
    children: [_cell(desc), _cell(total, bold: true), _cell("PIEZA")],
  );
