...
class _AssetsAdminPageState extends ConsumerState<AssetsAdminPage> {
                    ...
                    builder: (context) {
                      ...
                      return AnimatedBuilder(
                        ...
                        builder: (context, child) {
                          // Lógica de filtrado
                          final filteredClients = clientState.clients
                              .where(
                                (c) =>
                          ...
                          final filteredVehicles = vehicleState.vehicles
                              .where(
                                (v) =>
                                    v.brand!.toLowerCase().contains(
                                      _searchQuery.toLowerCase(),
                                    ) ||
                          ...
                          final filteredPress = pressState.press
                              .where(
                                (p) =>
                                    p.serie.toLowerCase().contains(
                                      _searchQuery.toLowerCase(),
                                    ) ||
                          ...
                          List<dynamic> activeItems = (tabController.index == 0)
                              ? filteredClients
                          ...
                          double dynamicHeight =
                              (activeItems.length * (isMobile ? 110.0 : 72.0)) +
                              80.0;
  ...
  Widget _buildStateContent(
    ...
    if (status == Status.error)
      return Center(child: Text(error ?? "Error al cargar"));
    if (items.isEmpty)
      return const Center(
        child: Text(
    ...
    return isMobile
        ? _buildMobileList(items, type)
        : _buildDesktopTable(items, type);
  }
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: Text(
              type == "cliente"
                  ? item.name
                  : (type == "vehiculo" ? item.licensePlate : item.serie),
  ...
  List<DataColumn> _buildColumns(String type) {
    final List<Map<String, dynamic>> cols = type == "cliente"
        ? [
            {"l": "Nombre", "i": Icons.person},
            {"l": "Compañía", "i": Icons.business},
            {"l": "Teléfono", "i": Icons.phone},
            {"l": "Acciones", "i": Icons.bolt},
          ]
        : type == "vehiculo"
        ? [
            {"l": "Marca", "i": Icons.directions_car},
            {"l": "Modelo", "i": Icons.info_outline},
            {"l": "Placa", "i": Icons.badge},
            {"l": "Acciones", "i": Icons.bolt},
          ]
        : [
            {"l": "Serie", "i": Icons.qr_code},
            {"l": "Tipo", "i": Icons.category},
            {"l": "Voltaje", "i": Icons.electric_bolt},
            {"l": "Acciones", "i": Icons.bolt},
          ];
    ...
    return PopupMenuButton<String>(
      ...
      itemBuilder: (context) => [
        ...
        const PopupMenuItem(
          ...
          child: Row(
            children: [
              Icon(Icons.edit_outlined, size: 18, color: Color(0xFFD32F2F)),
              SizedBox(width: 10),
              Text("Editar"),
            ],
          ),
  ...
  Widget _buildStateTable(Status status, String? error, Widget table) {
    ...
    if (status == Status.error)
      return Center(child: Text(error ?? "Error inesperado"));
    return table;
  }
}
