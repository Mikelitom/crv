...
class ConveyorReportTable extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConveyorReportTable> createState() => _ConveyorReportTableState();
}

class _ConveyorReportTableState extends ConsumerState<ConveyorReportTable> with ReportActionHandler {
  String searchQuery = '';
  String selectedStatus = 'Todos';

  // Mapa para guardar la versión seleccionada actualmente por cada reporte (reportId -> ConveyorHistoryModel)
  final Map<String, ConveyorHistoryModel> _selectedVersions = {};
  ...
  Color _getStatusColor(String state) {
    ...
    if (translated == 'En revisión') return Colors.amber.shade700;
    if (translated == 'Regresado') return Colors.purple;
    return Colors.green;
  }
  ...
  Widget _buildVersionSelector(BuildContext context, ConveyorHistoryModel currentItem, List<ConveyorHistoryModel> allReports) {
    ...
    final versionNumber = versionIndex != -1 ? versiones.length - versionIndex : 1;
    final versionText = currentItem.versionId.length >= 8 ? currentItem.versionId.substring(0, 8) : currentItem.versionId;

    if (versiones.length <= 1) {
      return Text(
        "Banda v$versionText... (v1)",
    ...
    return PopupMenuButton<ConveyorHistoryModel>(
      ...
      child: Wrap(
        ...
        children: [
          Text(
            "Banda v$versionText... (v$versionNumber)",
            style: const TextStyle(
              fontWeight: FontWeight.bold,
              color: kDarkText,
              fontSize: 13,
  ...
  Widget build(BuildContext context) {
    final allReports = widget.reports.where((r) => r is ConveyorHistoryModel).cast<ConveyorHistoryModel>().toList();

    final Map<String, ConveyorHistoryModel> uniqueMap = {};
    ...
    final filteredReports = activeReports.where((item) {
      final matchesSearch = searchQuery.isEmpty ||
          ...
          item.inspectorName.toLowerCase().contains(searchQuery.toLowerCase());
    ...
    final revisionCount = allReports.where((r) => _translateStatus(r.state) == 'En revisión').length;
    final approvedCount = allReports.where((r) => _translateStatus(r.state) == 'Aprobado').length;
    final returnedCount = allReports.where((r) => _translateStatus(r.state) == 'Regresado').length;
        ...
        return Column(
          ...
          children: [
                  ...
                  if (isWide) {
                    ...
                    return Column(
                      children: [
                        Row(
                          children: [
                            Expanded(child: counterCards[0]),
                            const SizedBox(width: 12),
                            Expanded(child: counterCards[1]),
                          ],
                        ),
                        const SizedBox(height: 12),
                        Row(
                          children: [
                            Expanded(child: counterCards[2]),
                            const SizedBox(width: 12),
                            Expanded(child: counterCards[3]),
                          ],
                        ),
                      ],
                    );
                  ...
                  SizedBox(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: "Buscar por folio, inspector...",
            ...
            const Padding(
              ...
              child: Text(
                "Historial de Reportes de Bandas",
                style: TextStyle(
                  fontSize: 18,
                ...
                child: ClipRRect(
                  ...
                  child: BaseTable(
                    ...
                    rows: filteredReports.map((item) {
                      ...
                      final inspectorInitial = item.inspectorName.isNotEmpty ? item.inspectorName[0].toUpperCase() : 'I';
  ...
  Widget _buildMobileView(
    BuildContext context,
    WidgetRef ref,
    List<ConveyorHistoryModel> filteredReports,
  ) {
    final allReports = widget.reports.where((r) => r is ConveyorHistoryModel).cast<ConveyorHistoryModel>().toList();
