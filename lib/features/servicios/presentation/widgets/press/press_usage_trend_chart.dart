...
class PressUsageTrendChart extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PressUsageTrendChart> createState() =>
      _PressUsageTrendChartState();
}
  ...
  List<FlSpot> _calculateSpots(List<dynamic> data, DateRange range) {
    ...
    for (int i = 0; i < days; i++) {
      ...
      int activeCount = data.where((p) {
        // Validamos que loan_date exista
        if (p['loan_date'] == null) return false;

        DateTime start = DateTime.parse(p['loan_date'].toString());

        // Si return_date es nulo, tomamos el momento actual (sigue activo)
        DateTime end = p['return_date'] != null
            ? DateTime.parse(p['return_date'].toString())
            : DateTime.now();

        return day.isAfter(start) && day.isBefore(end);
      }).length;

      double percentage = data.isNotEmpty
                    ...
                    LineChartData(
                      ...
                      titlesData: FlTitlesData(
                        ...
                        bottomTitles: AxisTitles(
                          sideTitles: SideTitles(
                            showTitles: true,
                            interval: isCompact ? 4 : 2,
                            getTitlesWidget: (v, _) => Text(
                              DateFormat('d MMM').format(
                                DateTime.now().subtract(
                                  Duration(
                                    days:
                                        _rangeToDays(_selectedRange) -
                                        1 -
                                        v.toInt(),
                                  ),
                                ),
  ...
  Widget _buildDropdown() => PopupMenuButton<DateRange>(
    child: Container(
      ...
      child: Row(
        children: [
          Text(
            _rangeToSpanish(_selectedRange),
    ...
    itemBuilder: (context) => DateRange.values
        .map((r) => PopupMenuItem(value: r, child: Text(_rangeToSpanish(r))))
        .toList(),
  );
}
