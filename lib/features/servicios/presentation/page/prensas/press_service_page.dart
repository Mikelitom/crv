...
class _PressServicePageState extends ConsumerState<PressServicePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredPress = state.press
        .where((p) => p.serie.toLowerCase().contains(searchQuery.toLowerCase()) || 
      ...
      floatingActionButton: LayoutBuilder(builder: (context, constraints) {
        return constraints.maxWidth <= 1000
            ? FloatingActionButton(
                ...
                child: Icon(showList ? Icons.close : Icons.list, color: Colors.white),
              )
        ...
        return Stack(
          children: [
            ...
            AnimatedPositioned(
              ...
              left: isWide ? 380 : 0, right: 0, top: 0, bottom: 0,
              child: Padding(
                padding: const EdgeInsets.all(24),
                child: selectedPress == null
                    ? _buildDashboard(usageData, filteredPress)
                    : PressServiceDetailView(key: ValueKey(selectedPress!.id), press: selectedPress!),
              ),
            ),
  ...
  Widget _buildDashboard(AsyncValue usageData, List<Press> filteredPress) {
    return SingleChildScrollView(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Dashboard de Prensas", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
          ...
          Wrap(
            ...
            children: [
              ...
              SizedBox(width: 380, child: usageData.when(
                loading: () => const Center(child: CircularProgressIndicator()),
                error: (err, _) => Text("Error: $err"),
                data: (data) => PressUsageTrendChart(data: data),
              )),
              SizedBox(width: 380, child: PendingMaintenanceWidget(onNavigateToPress: (pressId) {
  ...
  Widget _buildSidePanel(List<Press> filteredPress, Status status, bool isWide) {
    return Container(
      width: 350,
        ...
        children: [
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: const InputDecoration(hintText: "Buscar por placa...", prefixIcon: Icon(Icons.search, color: Color(0xFFC62828)), filled: true, fillColor: Color(0xFFF4F7FA), border: OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(16)), borderSide: BorderSide.none)),
            ),
          ),
          Expanded(
            child: status == Status.loading 
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (_, i) => _buildPressTile(filteredPress[i], isWide),
                ),
          ),
        ],
      ),
  ...
  Widget _buildPressTile(Press p, bool isWide) {
    return ListTile(
      leading: const CircleAvatar(backgroundColor: Color(0xFFF4F7FA), child: Icon(Icons.precision_manufacturing, color: Color(0xFFC62828))),
