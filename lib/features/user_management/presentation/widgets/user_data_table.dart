...
class UserDataTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DataTable(
      ...
      dataRowMaxHeight: 85,
      columns: ["ESTADO", "USUARIO", "ROL", "ACCIONES"]
          .map(
            (l) => DataColumn(
              label: Text(
                l,
      ...
      rows: List.generate(users.length, (index) {
        ...
        return DataRow(
          color: WidgetStateProperty.resolveWith<Color?>(
            (states) => isActive ? null : Colors.grey.withOpacity(0.05),
          ),
          cells: [
            DataCell(
              Switch(
      ...
      children: [
        CircleAvatar(
          ...
          child: Text(
            name[0],
            style: TextStyle(
              color: isActive ? const Color(0xFFC62828) : Colors.grey,
              fontWeight: FontWeight.bold,
            ),
          ),
        ),
  ...
  Widget _actionIcon(IconData icon, Color color, bool active) => Container(
    ...
    decoration: BoxDecoration(
      color: active ? color.withOpacity(0.1) : Colors.grey.withOpacity(0.1),
