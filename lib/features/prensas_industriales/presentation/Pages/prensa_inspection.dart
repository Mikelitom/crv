...
class _PrensaInspectionPageState extends ConsumerState<PrensaInspectionPage> {
  ...
  // --- VISTA PREVIA PDF CON LLAVES EN INGLÉS ---
  void _showPdfPreview(BuildContext context) {
    ...
    final Map<String, dynamic> pdfData = {
      ...
      "inspection_date": fechaActual,
      "area": state.area ?? "N/A",
      "tipo": state.selectedPress?.type ?? "N/A",
      "modelo": state.selectedPress?.model ?? "N/A",
      "volts": state.selectedPress?.voltz ?? "N/A",
      "serie": state.selectedPress?.serie ?? "N/A",
      // Seguimos la estructura de 'answers' del JSON para los items
      "items": templateItems.map((item) => {
        "cant": item.quantity ?? 0, // Para el PDF se suele usar corto
        "unidad": item.measureUnit,
        "descripcion": item.name,
        "status": item.status, // "good", "bad", etc.
        "observation": item.observation, 
        "foto_bytes": item.evidenceBefore.isNotEmpty ? item.evidenceBefore.first.bytes : null,
      }).toList(),
    ...
    try {
      ...
      for (var item in templateItems) {
        ...
        for (var media in mediaToProcess) {
          ...
          final fileExtension = evidenceFile.type == 'video' ? 'mp4' : 'jpg';
          final tempFile = File('${tempDir.path}/temp_${item.id}_${media['tag']}.$fileExtension');
          ...
          uploadResult.fold(
            (failure) => debugPrint("Error subiendo archivo: ${failure.message}"),
      ...
      result.fold(
        (f) => _showSnack("Error al guardar reporte: ${f.message}", Colors.red),
        (_) {
  ...
  Widget _buildScannerView() => Container(
    ...
    child: const Center(child: Icon(Icons.qr_code_scanner, color: Colors.white, size: 80))
  );
