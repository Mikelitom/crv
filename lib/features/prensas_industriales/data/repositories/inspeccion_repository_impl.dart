...
class InspeccionRepositoryImpl implements InspeccionRepository {
  ...
  Future<Either<Failure, List<ComponentItem>>> getInspectionTemplate() async {
    try {
      ...
      return const Left(UnknownFailure("Error al procesar formulario"));
    }
  }
  ...
  Future<Either<Failure, Press>> getPressBySerie(String serie) async {
    try {
      ...
      return const Left(UnknownFailure("Error al buscar serie"));
    }
  }
  ...
  Future<Either<Failure, String>> createPressReport(Map<String, dynamic> reportData) async {
    try {
      ...
      return Left(ServerFailure(e.message ?? "Error al enviar reporte"));
    } catch (e) {
...
Future<Either<Failure, List<LoanArea>>> getLoanAreas() async {
  try {
    final List<dynamic> data = await dataSource.getAllLoanAreas();
    final areas = data.map((json) => LoanArea.fromJson(json)).toList();
    return Right(areas);
  } on DioException catch (e) {
    return Left(ServerFailure(e.message ?? "Error al cargar talleres activos"));
  } catch (e) {
    return const Left(UnknownFailure("Error al procesar la lista de talleres"));
  }
}
  ...
  Future<Either<Failure, LoanArea>> createLoanArea(Map<String, String> data) async {
    try {
      ...
      return const Left(ServerFailure("Error al crear taller nuevo"));
    }
  }
