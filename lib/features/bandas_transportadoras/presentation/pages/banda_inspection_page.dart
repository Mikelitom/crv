...
class BandaInspectionPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<BandaInspectionPage> createState() =>
      _BandaInspectionPageState();
}
...
class _BandaInspectionPageState extends ConsumerState<BandaInspectionPage> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (widget.itemToEdit != null) {
        ...
        await ref
            ...
            .fetchDetail(widget.itemToEdit!.versionId);
  ...
  bool _validarDatosGenerales() {
    ...
    if (state.area.length < 2 ||
      ...
      _showSnack(
        "Error: Área, Transportador, Material, Granulometría y Presentar a deben tener al menos 2 caracteres",
        Colors.red,
      );
      return false;
    }
  ...
  Future<void> _guardarReporte({required bool esFinalizar}) async {
    ...
    try {
      ...
      for (var section in state.sections) {
        for (var component in section.components) {
          ...
          if (fixedIds.isEmpty &&
              ...
              component.observation.isEmpty &&
              component.comment.isEmpty)
            continue;

          final List<Map<String, dynamic>> evidenceList = [];
          final allFiles = [
            ...component.evidenceBefore,
            ...component.evidenceAfter,
          ];

          for (var evFile in allFiles) {
            // 🔥 CAMBIO AQUÍ: Si ya tiene path, es del servidor, NO la subimos de nuevo.
            if (evFile.path != null && evFile.path!.isNotEmpty) {
              ...
              final file = File(
                '${tempDir.path}/banda_${DateTime.now().microsecondsSinceEpoch}.jpg',
              );
      ...
      final rollersData =
          (state.isRodilleriaActive && filteredRollers.isNotEmpty)
          ? {"roller_notes": state.rollerNotes, "rollers": filteredRollers}
          : null;
      ...
      final mineName = state.selectedMine?.name ?? "GEN";
      final initials = _getInitials(mineName);
      final folio =
          "P${initials}${now.day.toString().padLeft(2, '0')}${now.month.toString().padLeft(2, '0')}${now.year.toString().substring(2)}${state.area.isNotEmpty ? state.area[0].toUpperCase() : "X"}${state.conveyor.isNotEmpty ? state.conveyor : "000"}";
      ...
      // 🔥 CAMBIO AQUÍ: Validación segura de ID para evitar el Null check operator
      final result = (notifier.isEditing && state.editingReportId != null)
          ? await ref
                .read(updateBandaReportUseCaseProvider)
                .call(state.editingReportId!, reportRequest)
          : await ref
                .read(createBandaReportUseCaseProvider)
                .call(reportRequest);
        ...
        child: Column(
          children: [
            ...
            if (state.sections.isNotEmpty || state.isRodilleriaActive)
              Container(key: _sectionKey, child: _buildActiveContent(state)),
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () {
                      ref
                          ...
                          .toggleRodilleria(true);
                      setState(
                        () => _currentSectionIndex = state.sections.length,
                      );
                    },
  ...
  Widget _buildActiveContent(BandaInspectionState state) {
    ...
    if (_currentSectionIndex < state.sections.length) {
      ...
      return BandaSectionTable(
        key: ValueKey(
          '${section.id}_${state.isLoading}',
        ), // 🔥 CLAVE: Obliga a redibujar si carga
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 10),
            Expanded(
              // 2. Expanded evita el overflow si el título es largo
              child: Text("Confirmar Envío", overflow: TextOverflow.ellipsis),
            ),
          ],
        ),
        content: const Text(
    ...
    style: ElevatedButton.styleFrom(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(
          25,
        ), // Radio igual al de tu botón ANTERIOR
      ),
      elevation: 0,
    ),
